...
export default function QRCodeTable({ qrCodes }: QRCodeTableProps) {
	...
	return (
		<div className="bg-white dark:bg-gray-800 rounded-xl shadow overflow-hidden">
			{/* Sort Info */}
			<div className="px-6 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 flex items-center justify-between">
				...
				<div className="flex items-center gap-2 text-sm">
					...
					<select
						...
						className="bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 rounded-md px-2 py-1 text-gray-700 dark:text-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
					>
						<option value="name">Nome</option>
						<option value="createdAt">Data de Criação</option>
						<option value="accessLogs">Acessos</option>
						<option value="targetUrl">URL Destino</option>
					</select>
					<button
					...
					>
						{sortDirection === "asc" ? (
							<IoChevronUp className="w-5 h-5 text-gray-600 dark:text-gray-300" />
						) : (
							<IoChevronDown className="w-5 h-5 text-gray-600 dark:text-gray-300" />
						)}
					</button>
				</div>
			</div>
			...
			<table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
				<thead className="bg-gray-50 dark:bg-gray-700">
					<tr>
						...
						<SortableHeader field="targetUrl">URL Destino</SortableHeader>
